...
//    /// Function that should send question request to server
//    /// - Parameters:
//    ///   - body: Request body that will be send to ApI
//    ///   - callback: Callback function that will be async executed once request is done.
//    func postRequestQuestion(body: NSDictionary, _ callback: @escaping (Result<(), Error>) -> Swift.Void) {
//        // Execute api request
//        service.executeRequestQuestion(body: body) { (result: RESTManager.ResponseResult<RESTManager.DiscardableServerModel>) in
//            switch result.result {
//            case .success:
//                print("Seccesfully send post request to server")
//                callback(.success( () ))
...
//    func getLengthQuestions(id: String, _ callback: @escaping (Result<([QuestionModel]), Error>)->Swift.Void) {
//        // Execute api request
//        service.executeGetLengthQuestions(id: id) { (result: RESTManager.ResponseResult<RESTManager.DiscardableServerModel>) in
//            switch result.result {
//            case .success:
//                if let dictionary = result.rawData?.toDictionaries() {
//                    let models = dictionary.compactMap{ QuestionModel($0) }
//                    print("Seccessfuly fetched lenth questions from server side")
//                    callback(.success(models))
//                }
//            case .failure(let error):
//                callback(.failure(error))
...
//
//
//    func getScoreboard(userId: String, categoryId: String, _ callback: @escaping (Result<([ScoreModel]), Error>)->Swift.Void) {
//        // Execute api request
//        service.executeGetScoreboard(userId: userId, categoryId: categoryId) { (result: RESTManager.ResponseResult<RESTManager.DiscardableServerModel>) in
//            switch result.result {
//            case .success:
//                if let dictionary = result.rawData?.toDictionaries() {
//                    let models = dictionary.compactMap( { ScoreModel($0)} )
//                    print("Seccessfully fatched scoreboard from server side")
//                    callback(.success(models))
//                }
//            case .failure(let error):
//                callback(.failure(error))
